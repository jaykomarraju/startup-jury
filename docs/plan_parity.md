...
TEST
  ...
  - You ARE adding a router (`signups.ts`) — add its probes to scripts/role-matrix.ts in the SAME
    ...
    (§8 Q28). The baseline after Wave 5 is 723/723 — confirm the number moved by exactly the
    probes you added.
  Green gate: npm run typecheck && npm run lint && npm test && npm run build && npm run test:e2e
  The suite is known non-deterministic under load (§8 Q32). Before blaming a red leg on your work,
  re-run the failing file alone and check `uptime`. A machine at load 40+ times client tests out at
  5 s. And gate client assertions on a POPULATED element, not on a heading the loading state also
  renders — that is a flake you write yourself, and `W5-A` wrote it before catching it.
