...
### `W6-A` — the three-tab sign-up workspace *(written by `W5-B`)*

> **Why `W5-B` writes this one rather than `W6-B`'s letter-mate.** `W6-A` is the direct consumer of
> everything `W5-B` built: the agreement templates, the signatory pool, the signing-method model and
> the countersign gate all have a model, an API and tests, and **no screen**. Two of `W5-B`'s §9 rows
> are addressed to this session by name. `W6-B` already has a prompt (written by `W4-D`); `W6-C` does
> not, and whoever writes it should read §8 Q50 first — "seats" means two different things.
>
> Wave 6's base branch is **`main` after Wave 5 integration** — fill the commit in when you know it.

```
You are running session W6-A — the three-tab sign-up workspace, and the founder's side of it — of
the ai.STARTUPJURY parity programme. You have no prior context.
...
READ FIRST (in this order, and nothing else)
  1. docs/plan_parity.md — §1 Ground rules, §2 Session protocol, §4 Testing, §8 Q56 and Q59 (the
     signing method's editable window, and who may touch it), then ONLY your entry for W6-A in §6.
  2. Your worklist:
       python3 docs/prototype/tools/findings.py \
         --grep "three-tab|Documents tab|Agreement tab|Founder tab|8\.3|signup workspace" --full
  3. docs/prototype/source/specs/incubator.html §8.3 — the tabs, the shared document set and the
     assignment gate are written there, and the written spec outranks the prototype (§1.1).
  4. ${TMPDIR:-/tmp}/sj-prototype-split/AISJ_IC_SuserV15/_scripts.js lines 2000–2300 ONLY — that is
     `suStage`/`suPanel` (the staff workspace), `suMethodCard` + `suAssignCard` (2260–2296) and
     `suwFounderMethod`/`suwFSubmit` (the founder side). Do NOT read the file whole (2,900 lines).
  5. **The API you are building a screen for already exists.** Read these, in this order:
       src/server/esign/routes.ts   — every route, with its gate and its refusal codes in comments
       src/shared/agreements.ts     — the vocabulary + the pure rules (import them, do not restate)
       migrations/0034_signups_and_documents.sql, 0035_agreements_and_signatures.sql, 0049_*.sql
     `test/worker/esign.test.ts` is the executable description of the behaviour.
  Do NOT read docs/PARITY-FINDINGS.md whole (1.4 MB). Do NOT read a prototype HTML whole.

BUILD
  1. **The staff workspace** (`SignupWorkspace.tsx`): spec §8.3's three tabs — Documents, Agreement,
     Founder — over ONE document set, not a copy per tab. Replace today's single
     "Complete sign-up" button (the `complete_signup` transition in src/pipeline/incubator.ts).
  2. **The signing-method card**, from `GET /api/esign/signups/:id/method`. It is editable while
     `method.editable`, and collapses to the locked line (`describeSigningMethod` + `lockReason`)
     otherwise. Do not re-derive the rule client-side — the view carries it.
  3. **The authorised-signatory picker**, from that response's `options`: an `optgroup` "By role"
     (`roleOptionLabel`) and one "Named individuals" (`userOptionLabel`). PUT `…/signatory`.
  4. **The countersign gate, in the UI.** Until a signatory is assigned the button is REPLACED by
     "Assign an authorised signatory to countersign." (`_scripts.js:2038`). The server already
     refuses it with 409 `no_signatory`; your job is to stop offering it. `describeCountersignRefusal`
     has all three sentences.
  5. **The PM assignment gate**: a Program Manager's workspace is READ-ONLY until a Super User or
     Admin assigns them (`signups.assigned_user_id`, 0034). Nothing reads that column yet.
  6. **The founder side** in FounderPortal.tsx: the shared document set to submit, the read-only
     ...
     the signature itself via `POST …/founder-signature`. A founder reaches ONLY their own record;
     the server enforces it, and your test must assert it.

CONSTRAINTS
  - Own only: src/client/routes/SignupWorkspace.tsx (new), src/server/routes/signups.ts (new), the
    sign-up path in src/client/routes/FounderPortal.tsx, and one line each in src/server/index.ts
    and src/shared/nav.ts if you add a screen. Need something else changed? Record it in §9.
  - **Do NOT modify src/server/esign/** or src/shared/agreements.ts.** They are complete and tested;
    if you need a field the API does not return, add it as a §9 request rather than editing, and say
    which assertion in test/worker/esign.test.ts would have to move.
  - `src/client/index.css`, `src/shared/roles.ts` and `src/client/App.tsx` are §2.2
    ...
    confirm that in §6 before you touch it.
  - migrations/: you own **0051** and only 0051 (0050 is `Wx-PWD`'s). Raise ALLOTMENT_CEILING in
    test/worker/migrations-w1b.test.ts to the wave's highest. **Check what exists first**: `signups`,
    `signup_documents`, `agreements`, `signatures` and `authorised_signatories` are all built and
    seeded — a finding's `REPO NONE` line predates Wave 1 (§9, `W5-B`'s last row).
  - §1.2: no card fields anywhere in this application.

TEST
  - E2E, the whole loop, one role at a time: admin configures a template → staff assigns a
    signatory → founder submits documents and signs → the method is now LOCKED on screen → staff
    countersigns → the deck is onboarded. Mutating shared rows? Run serially and restore what you
    found — `e2e/agreements.spec.ts` and `e2e/crm-sync.spec.ts` show the shape, including the
    `NAV = { timeout: 30_000 }` budget for navigation-gated assertions (§9).
  - Worker: the PM read-only gate (assigned → allowed, unassigned → refused), and founder isolation
    (another founder's record → 404, never 200).
  - Client: the method card in BOTH states (editable and locked), and the countersign button in both
    (offered and replaced by the sentence).
  - Green gate: npm run typecheck && npm run lint && npm test && npm run build && npm run test:e2e
    Plus `npm run roles` — you are adding a router, so add its probes to scripts/role-matrix.ts in
    the SAME commit. Write the probe body so an ALLOWED role still gets a 4xx (a ghost id, or a body
    that fails validation): the harness reports a write probe that returns 2xx as a finding.
    Pick a port from your session id and PROVE you own it with `lsof` BEFORE and AFTER starting
    `E2E_PORT=<port> npm run e2e:serve` (§2.3) — `npm run roles` exits 0 when it cannot connect, so
    a forgotten server reads as a pass. `W5-B` measured **735 / 735** on its own branch; `W5-A`
    adds probes too, so take the merged Wave 5 number from §7 rather than from here.
  - Before you believe a red run, check `uptime` and re-run the failing file alone (§8 Q28 / Q32).
    At load ~60 this suite's login page takes longer than Playwright's default expect timeout, and
    W5-B lost two full e2e runs to exactly that. No test should fail twice.
