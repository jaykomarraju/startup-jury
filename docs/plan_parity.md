...
| `W7-B` | `src/shared/crm.ts` *(`W3-D`'s)* | Spread `INTAKE_DETAIL_FIELDS` instead of `REQUIRED_INTAKE_FIELDS` into `APP_FIELDS.inbound`. The old name is kept as an alias, so nothing breaks today; it just no longer means "required" (§8 Q82). | any owner · 1 line |
