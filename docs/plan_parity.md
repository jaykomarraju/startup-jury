...
| `S3-ACCOUNT` | `e2e/roles.spec.ts:82` — *"admin opens Buy credits on the published credit packs"* (unowned) | **Its subject no longer exists, and the branch is RED on it by design.** Widening v3's seat flow to the incubator ADMIN (item 14; the client's 21-Sep row for My account reads **Superuser/Admin**) replaces that admin's `/app/billing` with **"Choose your seat"**, which has **no pay-as-you-go pack tab at all** — so `getByRole("tab", { name: "Pay-as-you-go credit packs" })` and `ac-plan-pack_50` have nothing to match. **Coverage was not dropped: it moved.** `e2e/account-purchase.spec.ts` (owned here) gained *"the VC admin's Buy credits still opens on the published credit packs"*, which carries the tab assertion, the `₹20,000` price and the §1.3 no-free-grant check onto the VC admin, who still has that screen. The exact diff, replacing the whole test body: rename to `admin opens Buy credits on the published seat catalogue`; `"Choose your plan"` → `"Choose your seat"`; **delete** the `Pay-as-you-go credit packs` tab assertion and the `ac-plan-pack_50` assertion; **add** `await expect(page.getByTestId("ac-tier-pro")).toBeVisible();`; **keep** `await expect(page.getByText("Added 20 credits")).toHaveCount(0);` — that §1.3 guard is what the test is really for. | 21-Sep wave integration |
| `S3-ACCOUNT` | `e2e/upload.spec.ts:164` — *"an admin's Buy credits lands on Choose your plan"* (unowned) | **The seventh assertion — neither §4 Q85 nor §12.4 counted it.** Both said six controls fail when the seat-flow gate widens (`accountOverlay.test.tsx` ×4 · `parity.spec.ts` · `roles.spec.ts`); this one makes seven, and it is the other half of the RED on this branch. It logs in as `nisha.kapoor@demo.startupjury.ai` (incubator admin), clicks Buy credits on the upload credits bar and asserts the destination heading — behaviour this change does not touch, only the heading moved. **Exact diff, two lines:** test name `an admin's Buy credits lands on Choose your plan` → `an admin's Buy credits lands on the seat screen`; and `await expect(page.getByRole("heading", { name: "Choose your plan" })).toBeVisible({ timeout: 30_000 });` → the same line with `"Choose your seat"`. Nothing else in the file is affected. **Take this row and the one above together** — either both land or the gate widening should not merge, because apart they leave the suite red. | 21-Sep wave integration |
| `S2-SETUP` | `src/client/routes/account/AccountScreens.tsx:2011-2024` *(`S3-ACCOUNT`'s, this wave)* — **the shared line §13 named** | **The V3 receipt's "What happens next?" list sends the buyer to two steps that no longer exist for them.** 21-Sep item 7 deletes the Set up wizard's Org type and Team steps for the incubator super user, so of the receipt's three numbered steps under *What happens next? Go to "Set up"*, (1) **Select your role** and (2) **Invite team members** are now unreachable from Set up — the wizard is Configure → Select. Step (3), Upload, is still correct. Exact diff, to apply as-is: replace the first item's text with `<b>Configure your programmes</b> — add the sectors, programmes and cohorts your workspace runs on`, and the second's with `<b>Invite team members</b> — add colleagues and assign their roles in <b>Admin console → Team &amp; roles</b>`. Both keep the `<b>` + em-dash shape the other two use, so the block's layout is untouched, and both are inside the same `[...]`.map() array — no other line of that file is affected. The sentence is prototype copy, so record the deviation beside item 13's in `plan_v3_superuser.md` §12. If `S3-ACCOUNT` has already shipped, this is a two-string edit for integration. | `S3-ACCOUNT`, else wave integration |
| `S2-SETUP` | `src/client/routes/admin/Branding.tsx` · `src/client/routes/admin/TeamRoles.tsx` *(unowned this wave — **already placed**, flagged per §2.2)* | **Item 7's two strandings, re-homed rather than raised**, because §13 required both resolved BEFORE the deletion and a request left unapplied would have shipped the deletion without them — which §9's own record says happens (four request patches sat unapplied for one to two waves). Nobody else in the 21-Sep wave owns either file: `S1` is the dashboard, `S3` `account/**`, `S4` `AreaWeights.tsx`, `S5` a new `help/**`. **`Branding.tsx`** gains an *Organisation name* field, the new writer for `branding.orgName` — only writer was the deleted Org type step, four readers are My Account's Workspace row, the account-invite email, the founder incomplete-deck email and every invoice's bill-to name. Held beside `brandingPatch`, never inside it, so `shared/branding.ts` is untouched and its three pinned test files do not move. **`TeamRoles.tsx`** gains a `SeatsCard` mounting the seat bar and the Buy-additional-seats flow — now shared at `src/client/routes/seats/BuySeats.tsx` — gated to `incubator`+`superuser`: §4 Q84's own answer, and the destination the deleted step's handoff card already pointed at. 10 tests added across `branding.test.tsx` and `teamRoles.test.tsx`, both negative-controlled; two real defects in the new code were caught that way and are recorded in `plan_v3_superuser.md` §12. | — (placed on `parity/S2-SETUP`) |

| `S1-DASH` | `test/client/queryPage.test.tsx` · `test/client/queryPageVc.test.tsx` — **placed, harness-only, flagged per §2.2** | `QueryPage` now reads the Assign screen's hand-off off router state (`useLocation().state.deckIds` — the §12.4 defect, where `AssignPage` has passed that state since V4-ROUTE and the page read it in 0 of 1,084 lines). Both suites mount the page with **no Router**, which `/app/query` never does, so every test in both files threw `useLocation() may be used only in the context of a <Router> component` — 23 failures, none of them about the change. Fix is the same two lines in each file's `renderPage()`: `import { MemoryRouter } from "react-router-dom";` and wrap `<QueryPage />` in `<MemoryRouter>` inside the existing `<StrictMode>`. **No assertion, fixture or mock is touched in either file.** Applied on `parity/S1-DASH` rather than left as a request, because the alternative was a branch red on 23 tests for however long the request sat — and the production-code alternative (splitting the hand-off into a child component behind `useInRouterContext()`) is test scaffolding leaking into shipped code. Nothing else in this wave touches the Query screen, so the conflict risk is nil. **Do not apply twice.** | `S1-DASH` (already on the branch) |
| `S1-DASH` | *(no file — a note for whoever runs the next wave's gate)* | **`npm run roles` cost ~12,800 `TIME_WAIT` entries on its own**, measured on this box: the harness logs in as all 13 seed users and probes every endpoint, and each probe is a server-side dispatch through the same `Miniflare.dispatchFetch` path the V4-ROUTE infra row identifies. It is not free and it is not only Playwright. Run it **before** e2e, or leave the ports time to drain between the two — they drop to double digits within a minute once nothing is running. Also measured: a sibling worktree's e2e run (`sj-S3-ACCOUNT`, port 5173) held ~12,200 of the 16,384 ports for the length of its run, so "never two e2e runs at once" is checkable from outside the worktree with `ps aux \| grep playwright` plus `netstat -an \| grep TIME_WAIT \| awk '{print $5}'` bucketed by remote port — which names the offending run rather than just its size. | — (informational) |
