...
TEST
  ...
  **e2e does not parallelise across worktrees (§9).** Two Playwright + miniflare stacks kill each
  ...
  Separately: gate client assertions on a POPULATED element, never on a heading the loading branch
  also renders. That is a flake you write yourself, and `W5-A` wrote it before catching it.
