...
export function StagePage({ config: base }: { config: StageConfig }) {
  ...
  const stageRows = useMemo<StageRow[]>(
    () =>
      (decks ?? [])
        ...
        .map((deck) => ({ deck, signup: signups[deck.id], extra: extras[deck.id] })),
    [decks, signups, extras, config],
  );
  ...
  function cell(column: StageColumn | StageCustomColumn, deck: DeckView): ReactNode {
    if (typeof column !== "string") {
      return column.render(stageRows.find((r) => r.deck.id === deck.id) ?? { deck }, ctx);
    }
    switch (column) {
      case "startup":
        return (
                ...
                <table className="w-full text-left" style={{ minWidth: config.minWidth ?? "44rem" }}>
                  <thead>
                    <tr className="text-fg-muted">
                      {columns.map((c) => (
                        <th key={columnId(c)} className="px-4 py-2.5 text-xs font-medium uppercase tracking-wide">
                          {header(c)}
                        </th>
                      ))}
                      {!config.readOnly && (
